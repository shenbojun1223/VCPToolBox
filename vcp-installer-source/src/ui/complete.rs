use crate::app::{App, Component, RuntimeComponent};
use ratatui::{
    Frame,
...
pub fn render(frame: &mut Frame, app: &App) {
    ...
    let lines = if let Some(result) = &app.install_result {
        ...
        lines.push(Line::from(vec![
            Span::styled("  [路径] 安装路径: ", Style::default().fg(Color::White)),
            Span::styled(
        ...
        // ====== 运行时环境 ======
        if !result.installed_runtimes.is_empty() {
            ...
            for rt in &result.installed_runtimes {
                let status = if rt.is_portable() {
                    "(Portable)"
                } else {
                    "(System)"
                };
                lines.push(Line::from(Span::styled(
                    format!("    [OK] {} {}", rt.display_name(), status),
        ...
        // ====== 已安装应用 ======
        if !result.installed_components.is_empty() {
            lines.push(Line::from(Span::styled(
                "  ====== 已安装应用 ======",
                Style::default()
                    .fg(Color::White)
        ...
        if let Some(backend) = &result.backend_start_script {
            lines.push(Line::from(vec![
                Span::styled("  [后端] 启动后端: ", Style::default().fg(Color::White)),
        ...
        if let Some(frontend) = &result.frontend_start_script {
            lines.push(Line::from(vec![
                Span::styled("  [前端] 启动前端: ", Style::default().fg(Color::White)),
        ...
        lines.push(Line::from(Span::styled(
            "  上下键/PgUp/PgDn 滚动  |  O 打开目录  |  Enter 配置向导  |  Q 退出",
