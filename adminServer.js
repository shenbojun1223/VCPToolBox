...
const LEGACY_ADMIN_PANEL_BACKUP_ROOT = path.join(
  __dirname,
  "AdminPanel-backup-20260408-201832"
);
const VUE_ADMIN_PANEL_INDEX = path.join(VUE_ADMIN_PANEL_ROOT, "index.html");

if (!existsSync(VUE_ADMIN_PANEL_INDEX)) {
  console.warn(
    `[AdminServer] Vue AdminPanel build not found: ${VUE_ADMIN_PANEL_INDEX}`
  );
  console.warn(
    '[AdminServer] Run "npm run build" inside AdminPanel-Vue before starting the admin server.'
  );
}
...
const adminAuth = (req, res, next) => {
  ...
  const isReadOnlyPath = readOnlyDashboardPaths.some((p) =>
  ...
  if (!ADMIN_USERNAME || !ADMIN_PASSWORD) {
    console.error(
      "[AdminServer] AdminUsername or AdminPassword not set in config.env."
    );
    if (
  ...
  if (
    ...
    // 🌟 防DDoS：无凭据访问独立计数，阈值更宽松（不影响正常 cookie 过期场景）
      ...
      if (accessInfo.count >= MAX_NO_CREDENTIAL_REQUESTS) {
        console.warn(
          `[AdminServer] IP ${clientIp} blocked for ${
            NO_CREDENTIAL_BLOCK_DURATION / 60000
          } min — excessive unauthenticated requests (${
            accessInfo.count
          }/${MAX_NO_CREDENTIAL_REQUESTS}).`
        );
        tempBlocks.set(clientIp, {
          expires: now + NO_CREDENTIAL_BLOCK_DURATION,
        });
        ...
        if (accessInfo.count % 10 === 0) {
          console.log(
            `[AdminServer] Unauthenticated access from IP: ${clientIp}. Count: ${accessInfo.count}/${MAX_NO_CREDENTIAL_REQUESTS}`
          );
        }
      }
    }
...
const dailyNoteRootPath =
  process.env.KNOWLEDGEBASE_ROOT_PATH || path.join(__dirname, "dailynote");
...
const localModules = [
  ...
  "tvs", // TVS 变量文件管理
  "placeholderExplorer", // 占位符索引/定位/预览/安全编辑
  "schedules", // 日程管理
...
const mockPluginManager = {
  ...
  loadPlugins: async () => {
    console.log(
      "[AdminServer] pluginManager.loadPlugins() called in admin process — skipped (use reload-notify to trigger main process reload)."
    );
  },
  hotReloadPluginsAndOrder: async () => {
    console.log(
      "[AdminServer] pluginManager.hotReloadPluginsAndOrder() called in admin process — proxying to main process is recommended."
    );
    return [];
  },
  ...
  getPlaceholderValue: (key) =>
    `[Placeholder ${key} not available in admin process]`,
...
const localOptions = {
  ...
  triggerRestart: (code = 1) => {
    console.log(
      `[AdminServer] Restarting admin process (exit code: ${code})...`
    );
...
for (const moduleName of localModules) {
  try {
    ...
    if (DEBUG_MODE)
      console.log(`[AdminServer] Mounted local module: ${moduleName}`);
  } catch (error) {
    console.error(
      `[AdminServer] Failed to load local module "${moduleName}":`,
      error.message
    );
  }
}
  ...
  const profileReq = http.request(
    ...
    (profileRes) => {
      ...
      profileRes.on("end", () => {
        ...
        const contentType =
          profileRes.headers["content-type"] || "application/json";
  ...
  const trainingReq = http.request(
    ...
    (trainingRes) => {
      ...
      trainingRes.on("end", () => {
        ...
        const contentType =
          trainingRes.headers["content-type"] || "application/json";
...
app.post("/admin_api/server/restart", async (req, res) => {
  ...
  const restartReq = http.request(
    ...
    (restartRes) => {
      ...
      restartRes.on("end", () => {
        console.log(
          `[AdminServer] Main process restart response: ${restartRes.statusCode}`
        );

        if (res.headersSent) return;

        const contentType =
          restartRes.headers["content-type"] || "application/json";
...
app.use("/admin_api", (req, res, next) => {
  ...
  if (DEBUG_MODE)
    console.log(`[AdminServer Proxy] ${req.method} ${fullPath} -> ${proxyUrl}`);
  ...
  const incomingContentType = String(
...
app.post("/admin_api/config/main/reload-notify", async (req, res) => {
  try {
    ...
    notifyReq.on("error", (err) => {
      ...
      res.json({
        success: true,
        message: "配置已保存到文件，但主服务通知失败（可能需要手动重启）。",
        warning: err.message,
      });
    });
...
app.listen(ADMIN_PORT, () => {
  ...
  console.log(
    `[AdminServer] 管理面板地址: http://localhost:${ADMIN_PORT}/AdminPanel/`
  );
  console.log(`[AdminServer] Vue 面板目录: ${VUE_ADMIN_PANEL_ROOT}`);
  console.log(
    `[AdminServer] Legacy 备份目录: ${LEGACY_ADMIN_PANEL_BACKUP_ROOT}`
  );
