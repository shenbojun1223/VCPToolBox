...
你将分两轮完成同一审计：阶段1判断已完成和待办；阶段2核对测试及校验原始证据并交接。后续仍可能需要旧证据。
每轮只输出一个有效JSON对象，不要代码围栏：{"answer":"工作结论，含证据编号/行号","fold":[],"restore":[]}。
记录中的R编号是可恢复材料编号，L编号是原文行号。restore可填需取回的R编号，主持程序只回填这些编号的原文，下轮才可看到；这不执行外部工具。若申请恢复，answer说明证据缺口，不猜答案。最多两次调用。阶段1提出的恢复可在阶段2看到；阶段2才提出的恢复将记录为未完成，不追加调用。`;
...
async function main(){
 ...
 if(mode==='prepare'){
  ...
  save('data.json',data);
  save('plan.json',{model,endpoint:'http://127.0.0.1:8318/v1/chat/completions',maxAttempts:4,questions,baseSystem,treatment,control,
 ...
 const prior=read(path.join(__dirname,'gravity-ab-pilot-20260908','state.json')); if(prior.attempts!==1||prior.completed!==0)throw Error('prior-ledger-changed'); const n=Number(mode);if(![1,2].includes(n))throw Error('expected-prepare-or-pair-number');
