...
| 属性 | 类型 | 说明 |
|------|------|------|
| `version` | `string` | 接口版本号（当前 `'1.1'`），用于兼容性检查 |
...
#### `searchDiary(diaryNames, queryVector, k?)` — async

在指定日记本索引中执行基础 KNN 检索。该接口保留用于向后兼容，不启用 TagMemo。

#### `retrieveDiary(options)` — async

在指定日记本索引中执行结构化检索，可选择启用 TagMemo 浪潮增强、查询级测地线重排和智能语义去重。接口返回候选对象，不生成展示 Markdown，也不会重新向量化日记文档。

| 选项 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `diaryNames` | `string \| string[]` | — | 日记本名称或虚拟联合索引成员 |
| `queryVector` | `Array \| Float32Array` | — | 已生成的查询向量 |
| `k` | `number` | `10` | 最大候选 chunk 数 |
| `tagMemo` | `boolean` | `false` | 是否启用 TagMemo 浪潮增强 |
| `tagWeight` | `number` | 动态计算 | 显式 Tag 权重 |
| `geodesicRerank` | `boolean` | `false` | 是否启用查询级测地线重排 |
| `deduplicate` | `boolean` | `false` | 是否执行智能语义去重 |
| `userText` | `string` | `''` | 动态参数计算使用的用户文本 |
| `aiText` | `string` | `''` | 动态参数计算使用的 AI 文本 |
| `coreTags` | `Array` | `[]` | 显式核心 Tag 或幽灵节点 |
...
桥接层只运行一次 TagBoost，并通过 `preparedBoostResult` 将查询级增强向量和能量场传给 KnowledgeBaseManager，避免重复执行浪潮管线。TagMemo 不可用或执行失败时会自动退化为普通 KNN，并通过 `fallbackReason` 说明原因。
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| 1.1 | 2026-07-11 | 新增 `retrieveDiary(options)`，公开结构化 TagMemo 浪潮检索、测地线重排与去重能力 |
