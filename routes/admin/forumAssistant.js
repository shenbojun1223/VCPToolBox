...
module.exports = function (options) {
    ...
    function ensureModule(res) {
        const mod = getModule();
        if (!mod) {
            res.status(503).json({ error: 'VCPForumAssistant 插件未加载' });
            return null;
        }
        return mod;
    }
    ...
    router.post('/forum-assistant/trigger', async (req, res) => {
        try {
            ...
            if (req.body.agentName) {
                ...
                if (!matchedTask) {
                    return res.status(404).json({ error: `未找到 Agent "${req.body.agentName}" 对应的论坛任务` });
                }
    ...
    router.get('/forum-assistant/tasks', (req, res) => {
        try {
            const mod = ensureModule(res);
            if (!mod) return;
            ...
            console.error('[ForumAssistant Route] listTasks error:', e);
            res.status(500).json({ error: e.message });
        }
    });

    router.post('/forum-assistant/tasks', async (req, res) => {
        try {
            const mod = ensureModule(res);
            if (!mod) return;
            ...
            console.error('[ForumAssistant Route] createTask error:', e);
            res.status(500).json({ error: e.message });
        }
    });

    router.patch('/forum-assistant/tasks/:taskId', async (req, res) => {
        try {
            const mod = ensureModule(res);
            if (!mod) return;
            ...
            console.error('[ForumAssistant Route] updateTask error:', e);
            res.status(500).json({ error: e.message });
        }
    });

    router.delete('/forum-assistant/tasks/:taskId', async (req, res) => {
        try {
            const mod = ensureModule(res);
            if (!mod) return;
            ...
            console.error('[ForumAssistant Route] deleteTask error:', e);
            res.status(500).json({ error: e.message });
        }
    });
