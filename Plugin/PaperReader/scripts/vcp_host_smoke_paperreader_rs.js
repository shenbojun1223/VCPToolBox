...
const forceDeterministic =
  process.env.PAPERREADER_SMOKE_REAL === "1" ? "0" : "1";
...
async function execViaHost(pluginManager, input) {
  const output = await pluginManager.executePlugin(
    "PaperReader",
    JSON.stringify(input)
  );
  if (!output || output.status !== "success") {
    const err =
      (output && (output.error || output.pluginStderr)) || "unknown error";
...
async function execViaDirect(entryCommand, input, extraEnv) {
  ...
  try {
    ...
    throw new Error(
      `Direct invoke produced non-JSON stdout. exit=${exitCode}, stderr=${stderr.slice(
        0,
        200
...
function registerPaperReaderForSmoke(pluginManager) {
  const enabledManifestPath = path.join(pluginRoot, "plugin-manifest.json");
  const blockedManifestPath = path.join(
    pluginRoot,
    "plugin-manifest.json.block"
  );
  const manifestPath = fs.existsSync(enabledManifestPath)
    ? enabledManifestPath
    : blockedManifestPath;

  if (!fs.existsSync(manifestPath)) {
    throw new Error(
      `PaperReader manifest not found. Checked: ${enabledManifestPath}, ${blockedManifestPath}`
    );
  }

  const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
  manifest.basePath = pluginRoot;
  manifest.pluginSpecificEnvConfig = loadDotEnv(
    path.join(pluginRoot, "config.env")
  );
...
function assertCommandSurface(label, expectedSet, actualSet) {
  ...
  if (missing.length || extra.length) {
    throw new Error(
      `${label} command surface mismatch. missing=${
        missing.join(",") || "none"
...
async function waitForRunCompletion(pluginManager, runId, timeoutMs) {
  ...
  while (Date.now() - started < timeoutMs) {
    polls += 1;
    const state = await execViaHost(
      pluginManager,
      makeEnvelope("get_run_state", { run_id: runId })
    );
    lastState = state;
    ...
    if (status === "failed" || status === "aborted") {
      throw new Error(
        `Research run ${runId} entered terminal status ${status}`
      );
    }
    await new Promise((resolve) => setTimeout(resolve, 1000));
  }
  throw new Error(
    `Timed out waiting for async run ${runId}. Last state: ${JSON.stringify(
      lastState?.data?.run_state || {}
    )}`
  );
}
...
async function main() {
  if (!pdfPath) {
    throw new Error(
      'Usage: node scripts/vcp_host_smoke_paperreader_rs.js "<absolute-pdf-path>"'
    );
  }
  ...
  const bootstrap = await execViaHost(
    pluginManager,
    makeEnvelope("bootstrap_workspace", {})
  );
  const runtime = await execViaHost(
    pluginManager,
    makeEnvelope("describe_runtime", {})
  );
  const health = await execViaHost(
    pluginManager,
    makeEnvelope("get_health_snapshot", {})
  );
  const supportedCommands = new Set(runtime?.data?.supported_commands || []);
  assertCommandSurface(
    "host describe_runtime",
    manifestCommandIds,
    supportedCommands
  );

  const ingest = await execViaHost(
    pluginManager,
    makeEnvelope("IngestPDF", {
      filePath: pdfPath,
  ...
  if (!documentId) {
    throw new Error(
      `IngestPDF did not return document_id. keys=${Object.keys(
        ingest?.data || {}
      ).join(",")}`
    );
  }

  const canonicalIngest = await execViaHost(
    pluginManager,
    makeEnvelope("ingest_source", {
      source_text:
  ...
  if (canonicalIngest?.data?.document_id !== canonicalDocumentId) {
    throw new Error(
      `ingest_source did not return the requested document_id. payload=${JSON.stringify(
        canonicalIngest?.data || {}
      )}`
    );
  }

  const refresh = await execViaHost(
    pluginManager,
    makeEnvelope("refresh_ingestion", {
      document_id: canonicalDocumentId,
    })
  );
  if (refresh?.data?.refreshed !== true) {
    throw new Error(
      `refresh_ingestion did not confirm refresh. payload=${JSON.stringify(
        refresh?.data || {}
      )}`
    );
  }

  const read = await execViaHost(
    pluginManager,
    makeEnvelope("Read", {
      paperId: documentId,
      goal: "总结核心贡献、方法和实验设计",
  ...
  const canonicalRead = await execViaHost(
    pluginManager,
    makeEnvelope("read_document", {
      document_id: canonicalDocumentId,
  ...
  if (!canonicalRead?.data?.reading_state_ref) {
    throw new Error(
      `read_document did not return reading_state_ref. payload=${JSON.stringify(
        canonicalRead?.data || {}
      )}`
    );
  }

  const resumeRead = await execViaHost(
    pluginManager,
    makeEnvelope("resume_read", {
      document_id: canonicalDocumentId,
  ...
  if (!resumeRead?.data?.reading_state_ref) {
    throw new Error(
      `resume_read did not return reading_state_ref. payload=${JSON.stringify(
        resumeRead?.data || {}
      )}`
    );
  }
  ...
  const recursiveParagraph =
    "This paragraph exists to exercise recursive single-document reading. It should be long enough and split into many blocks so the SegmentSet yields multiple segments.";
  const recursiveText = Array.from(
    { length: 60 },
    (_, idx) =>
      `Section ${idx + 1}. ${recursiveParagraph} ${recursiveParagraph}`
  ).join("\n\n");
  const recursiveIngest = await execViaHost(
    pluginManager,
    makeEnvelope("ingest_source", {
  ...
  if (recursiveIngest?.data?.document_id !== recursiveDocumentId) {
    throw new Error(
      `recursive ingest_source did not return the requested document_id. payload=${JSON.stringify(
        recursiveIngest?.data || {}
      )}`
    );
  }

  const recursiveRead = await execViaHost(
    pluginManager,
    makeEnvelope("read_document", {
      document_id: recursiveDocumentId,
  ...
  if (!recursiveRead?.data?.global_map_ref) {
    throw new Error(
      `recursive read_document did not return global_map_ref. payload=${JSON.stringify(
        recursiveRead?.data || {}
      )}`
    );
  }
  if (
    !Array.isArray(recursiveRead?.data?.recursive_artifact_refs) ||
    recursiveRead.data.recursive_artifact_refs.length === 0
  ) {
    throw new Error(
      `recursive read_document did not produce intermediate recursive_artifact_refs. payload=${JSON.stringify(
        recursiveRead?.data || {}
      )}`
    );
  }
  if (enableRecursiveCritic) {
    const refs = (recursiveRead?.data?.recursive_artifact_refs || []).map(
      (ref) => String(ref)
    );
    if (!refs.some((ref) => ref.endsWith("global_map.critic.md"))) {
      throw new Error(
        `recursive critic smoke did not produce global_map.critic.md. refs=${refs.join(
          ","
        )}`
      );
    }
    if (
      !refs.some((ref) => ref.includes("group_") && ref.endsWith(".critic.md"))
    ) {
      throw new Error(
        `recursive critic smoke did not produce group_*.critic.md. refs=${refs.join(
          ","
        )}`
      );
    }
  ...
  const retrieve = await execViaHost(
    pluginManager,
    makeEnvelope("retrieve_evidence", {
      document_id: canonicalDocumentId,
  ...
  if (!retrieve?.data?.retrieval_hits_ref) {
    throw new Error(
      `retrieve_evidence did not return retrieval_hits_ref. payload=${JSON.stringify(
        retrieve?.data || {}
      )}`
    );
  }

  const evidencePack = await execViaHost(
    pluginManager,
    makeEnvelope("build_evidence_pack", {
      document_id: canonicalDocumentId,
      query_text: "canonical smoke evidence",
    })
  );
  if (!evidencePack?.data?.evidence_pack_ref) {
    throw new Error(
      `build_evidence_pack did not return evidence_pack_ref. payload=${JSON.stringify(
        evidencePack?.data || {}
      )}`
    );
  }

  const trace = await execViaHost(
    pluginManager,
    makeEnvelope("trace_claim_in_document", {
      document_id: canonicalDocumentId,
      claim_text:
        "Canonical smoke document includes evidence-backed statements.",
    })
  );

  const audit = await execViaHost(
    pluginManager,
    makeEnvelope("audit_document", {
      document_id: canonicalDocumentId,
    })
  );
  if (!audit?.data?.audit_report_ref) {
    throw new Error(
      `audit_document did not return audit_report_ref. payload=${JSON.stringify(
        audit?.data || {}
      )}`
    );
  }

  const collection = await execViaHost(
    pluginManager,
    makeEnvelope("ingest_collection", {
  ...
  const collectionIds = (collection?.data?.document_ids || []).map((value) =>
    String(value)
  );
  if (collectionIds.length !== 3) {
    throw new Error(
      `ingest_collection did not produce three document_ids. payload=${JSON.stringify(
        collection?.data || {}
      )}`
    );
  }

  const survey = await execViaHost(
    pluginManager,
    makeEnvelope("survey_collection", {
      collection_id: collectionId,
  ...
  if (!survey?.data?.collection_map_ref) {
    throw new Error(
      `survey_collection did not return collection_map_ref. payload=${JSON.stringify(
        survey?.data || {}
      )}`
    );
  }

  const comparison = await execViaHost(
    pluginManager,
    makeEnvelope("compare_documents", {
      collection_id: collectionId,
      document_ids: collectionIds,
    })
  );
  if (comparison?.data?.comparison_table?.pair_count !== 3) {
    throw new Error(
      `compare_documents did not produce all pairwise comparisons. payload=${JSON.stringify(
        comparison?.data || {}
      )}`
    );
  }

  const conflictAudit = await execViaHost(
    pluginManager,
    makeEnvelope("audit_collection_conflicts", {
      collection_id: collectionId,
      document_ids: collectionIds,
  ...
  if (!conflictAudit?.data?.conflict_report_ref) {
    throw new Error(
      `audit_collection_conflicts did not return conflict_report_ref. payload=${JSON.stringify(
        conflictAudit?.data || {}
      )}`
    );
  }

  const collectionCompare = await execViaHost(
    pluginManager,
    makeEnvelope("synthesize_collection", {
      collection_id: collectionId,
      document_ids: collectionIds,
      mode: "compare",
  ...
  if (collectionCompare?.data?.comparison_table?.document_count !== 3) {
    throw new Error(
      `synthesize_collection(compare) did not keep all documents. payload=${JSON.stringify(
        collectionCompare?.data || {}
      )}`
    );
  }
  if (collectionCompare?.data?.comparison_table?.pair_count !== 3) {
    throw new Error(
      `synthesize_collection(compare) did not create all pairwise comparisons. payload=${JSON.stringify(
        collectionCompare?.data || {}
      )}`
    );
  }
  const collectionSynthesis = await execViaHost(
    pluginManager,
    makeEnvelope("synthesize_collection", {
      collection_id: collectionId,
      document_ids: collectionIds,
  ...
  const plan = await execViaHost(
    pluginManager,
    makeEnvelope("plan_research", {
      collection_id: collectionId,
      document_ids: collectionIds,
  ...
  for (const nodeId of ["compare", "conflict_audit"]) {
    if (!plannedNodes.includes(nodeId)) {
      throw new Error(
        `plan_research did not include required node ${nodeId}. payload=${JSON.stringify(
          plan?.data || {}
        )}`
      );
    }
  ...
  const graphRef = (plan?.data?.artifact_refs || []).find((ref) =>
    String(ref).replace(/\\/g, "/").endsWith("/graph.json")
  );
  if (!graphRef) {
    throw new Error(
      `plan_research did not return graph.json artifact ref. payload=${JSON.stringify(
        plan?.data || {}
      )}`
    );
  }
  const graphArtifact = await execViaHost(
    pluginManager,
    makeEnvelope("get_artifact", { artifact_path: graphRef })
  );
  const graphNodes = Object.keys(graphArtifact?.data?.data?.nodes || {});
  for (const nodeId of ["compare", "conflict_audit"]) {
    if (!graphNodes.includes(nodeId)) {
      throw new Error(
        `graph.json did not materialize required node ${nodeId}. payload=${JSON.stringify(
          graphArtifact?.data || {}
        )}`
      );
    }
  ...
  if (accepted?.data?.status !== "accepted") {
    throw new Error(
      `run_research_graph did not return accepted. payload=${JSON.stringify(
        accepted?.data || {}
      )}`
    );
  }
  ...
  const events = await execViaHost(
    pluginManager,
    makeEnvelope("stream_run_events", { run_id: runId, cursor: 0, limit: 100 })
  );
  const runState = await execViaHost(
    pluginManager,
    makeEnvelope("get_run_state", { run_id: runId })
  );
  const workspaceState = await execViaHost(
    pluginManager,
    makeEnvelope("get_workspace_state", { run_id: runId })
  );
  const artifacts = await execViaHost(
    pluginManager,
    makeEnvelope("list_artifacts", { run_id: runId })
  );
  const artifactPaths = (artifacts?.data?.artifacts || []).map((item) =>
    String(item?.path || "").replace(/\\/g, "/")
  );
  for (const suffix of [
    "/nodes/compare/result.json",
    "/nodes/conflict_audit/result.json",
  ]) {
    if (!artifactPaths.some((artifactPath) => artifactPath.endsWith(suffix))) {
      throw new Error(
        `run artifacts did not include ${suffix}. payload=${JSON.stringify(
          artifacts?.data || {}
        )}`
      );
    }
  ...
  const artifact = await execViaHost(
    pluginManager,
    makeEnvelope("get_artifact", { artifact_path: runStateRef })
  );

  const cancelPlan = await execViaHost(
    pluginManager,
    makeEnvelope("plan_research", {
  ...
  if (!cancelRunId) {
    throw new Error(
      `cancel smoke plan_research did not return run_id. payload=${JSON.stringify(
        cancelPlan?.data || {}
      )}`
    );
  }
  ...
  if (cancelAccepted?.data?.status !== "accepted") {
    throw new Error(
      `cancel smoke run_research_graph did not return accepted. payload=${JSON.stringify(
        cancelAccepted?.data || {}
      )}`
    );
  }
  const cancelled = await execViaHost(
    pluginManager,
    makeEnvelope("cancel_run", {
      run_id: cancelRunId,
  ...
  if (!["aborted", "partial"].includes(cancelled?.data?.status)) {
    throw new Error(
      `cancel_run returned unexpected status. payload=${JSON.stringify(
        cancelled?.data || {}
      )}`
    );
  }
  const cancelState = await execViaHost(
    pluginManager,
    makeEnvelope("get_workspace_state", { run_id: cancelRunId })
  );

  const reset = await execViaHost(
    pluginManager,
    makeEnvelope("reset_run", { run_id: runId })
  );
  if (reset?.data?.status !== "reset") {
    throw new Error(
      `reset_run returned unexpected status. payload=${JSON.stringify(
        reset?.data || {}
      )}`
    );
  }
  const resetState = await execViaHost(
    pluginManager,
    makeEnvelope("get_workspace_state", { run_id: runId })
  );
  if (resetState?.data?.run_state?.status !== "pending") {
    throw new Error(
      `reset_run did not return run to pending state. payload=${JSON.stringify(
        resetState?.data || {}
      )}`
    );
  }

  const resumedRun = await execViaHost(
    pluginManager,
    makeEnvelope("resume_research_graph", { run_id: runId })
  );
  if (resumedRun?.data?.status !== "completed") {
    throw new Error(
      `resume_research_graph did not return completed. payload=${JSON.stringify(
        resumedRun?.data || {}
      )}`
    );
  }
  const resumedState = await execViaHost(
    pluginManager,
    makeEnvelope("get_run_state", { run_id: runId })
  );
  if (resumedState?.data?.run_state?.status !== "completed") {
    throw new Error(
      `resume_research_graph did not restore run to completed state. payload=${JSON.stringify(
        resumedState?.data || {}
      )}`
    );
  }

  const directRuntime = await execViaDirect(
    entryCommand,
    makeEnvelope("describe_runtime", {}),
    directEnv
  );
  const directSupportedCommands = new Set(
    directRuntime?.data?.supported_commands || []
  );
  assertCommandSurface(
    "direct describe_runtime",
    manifestCommandIds,
    directSupportedCommands
  );
  const directRefresh = await execViaDirect(
    entryCommand,
    makeEnvelope("refresh_ingestion", {
      document_id: canonicalDocumentId,
    }),
  ...
  const directResumeRead = await execViaDirect(
    entryCommand,
    makeEnvelope("resume_read", {
      document_id: canonicalDocumentId,
    }),
  ...
  const directAudit = await execViaDirect(
    entryCommand,
    makeEnvelope("audit_document", {
      document_id: canonicalDocumentId,
    }),
  ...
  const directSurvey = await execViaDirect(
    entryCommand,
    makeEnvelope("survey_collection", {
      collection_id: collectionId,
      document_ids: collectionIds,
  ...
  const directCompare = await execViaDirect(
    entryCommand,
    makeEnvelope("compare_documents", {
      collection_id: collectionId,
      document_ids: collectionIds,
  ...
  const directConflictAudit = await execViaDirect(
    entryCommand,
    makeEnvelope("audit_collection_conflicts", {
      collection_id: collectionId,
      document_ids: collectionIds,
  ...
  const directReadDeep = await execViaDirect(
    entryCommand,
    makeEnvelope("ReadDeep", {
      paperId: documentId,
      goal: "Direct stdio deep-focus compatibility check",
    }),
    directEnv
  );
  const directRunState = await execViaDirect(
    entryCommand,
    makeEnvelope("get_run_state", { run_id: runId }),
    directEnv
  );
  const directWorkspaceState = await execViaDirect(
    entryCommand,
    makeEnvelope("get_workspace_state", { run_id: runId }),
    directEnv
  );
  ...
  const summary = {
    ...
    direct: {
      ...
      manifest_surface_matches_runtime: [...manifestCommandIds].every(
        (command) => directSupportedCommands.has(command)
      ),
    },
    ids: {
      document_id: documentId,
    ...
    refs: {
      ...
      direct_workspace_state_ref:
        directWorkspaceState?.data?.run_state_ref || null,
      comparison_table_ref:
        collectionCompare?.data?.comparison_table_ref || null,
    },
  };
...
main().catch((error) => {
  process.stderr.write(
    `[smoke] FAILED: ${error && error.stack ? error.stack : String(error)}\n`
  );
  process.exit(1);
});
