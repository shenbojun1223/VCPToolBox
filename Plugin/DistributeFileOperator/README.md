...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `ALLOWED_DIRECTORIES` | 允许操作的目录白名单（逗号分隔；相对路径以插件目录为基准），为空则拒绝所有目录 | `../..` |
...
| `MAX_FILE_SIZE` | 允许操作的最大文件大小（字节） | 20971520 (20MB) |
...
4. **路径安全**：所有操作都会验证路径是否在 ALLOWED_DIRECTORIES 白名单内；未配置时默认拒绝
