...
function canonicalizePolicyPath(targetPath) {
  ...
  try {
    const realAncestor = fsSync.realpathSync.native(existingAncestor);
    return path.resolve(realAncestor, path.relative(existingAncestor, resolvedPath));
  } catch {
    return resolvedPath;
  }
}
...
function isPathWithin(basePath, targetPath) {
  ...
  const base = normalizeCase(canonicalizePolicyPath(basePath));
  const target = normalizeCase(canonicalizePolicyPath(targetPath));
...
function isPathAllowed(targetPath, operationType = 'generic') {
  if (ALLOWED_DIRECTORIES.length === 0) {
    debugLog('No ALLOWED_DIRECTORIES configured. Access denied by default.', { targetPath, operationType });
    return false;
  }

  const isInAllowedDir = ALLOWED_DIRECTORIES.some(allowedDir => {
    const resolvedAllowedDir = path.isAbsolute(allowedDir)
      ? allowedDir
      : path.resolve(__dirname, allowedDir);
    return isPathWithin(resolvedAllowedDir, targetPath);
  });
  ...
  debugLog('Access denied. Path is outside allowed directories.', { targetPath, operationType });
  return false;
}
