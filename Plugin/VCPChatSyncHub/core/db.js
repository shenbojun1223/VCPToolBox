...
function getDesktopConfigIndex(id, type) {
  if (!db) return null;
  return db
    .prepare("SELECT * FROM desktop_config_index WHERE id = ? AND type = ?")
    .get(id, type);
}

function getDesktopConfigs() {
  if (!db) return [];
  return db.prepare("SELECT * FROM desktop_config_index").all();
}

function upsertDesktopConfigIndex(id, type, filePath, hash, updatedAt = Date.now()) {
  if (!db) return;
  db.prepare(`
    INSERT INTO desktop_config_index (id, type, file_path, hash, updated_at, deleted_at)
    VALUES (?, ?, ?, ?, ?, NULL)
    ON CONFLICT(id, type) DO UPDATE SET
      file_path = excluded.file_path,
      hash = excluded.hash,
...
function softDeleteEntityIndex(id, type, deletedAt = Date.now()) {
  ...
  return db.prepare(sql).run({ deletedAt, id, type });
}
    ...
    return db
      .prepare(
        `UPDATE message_index
         SET deleted_at = CASE
           WHEN deleted_at IS NULL THEN @deletedAt ELSE MIN(deleted_at, @deletedAt) END
         WHERE topic_id = @topicId AND msg_id = @msgId`,
      )
