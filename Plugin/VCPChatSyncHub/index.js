...
async function initializeRoutes(app, pluginConfig, projectBasePath) {
  ...
  if (!syncToken || syncToken === "your_super_secret_token_here") {
    console.error(
      "[VCPChatSyncHub] MobileSyncToken is required. Copy config.env.example to config.env and set a strong token.",
    );
    return;
  }
  ...
  logger.logInfo("system", `中心同步数据目录: ${appDataPath}`);
}
...
function shutdown() {
  if (activeWatcher) {
    void activeWatcher.close().catch(() => {});
    activeWatcher = null;
  }
  if (cleanupTimer) {
...
function registerRoutes(app, pluginConfig, projectBasePath) {
  // PluginManager's legacy route hook is synchronous and does not await a
  // returned Promise, so contain asynchronous initialization errors here.
  void initializeRoutes(app, pluginConfig, projectBasePath).catch((error) => {
