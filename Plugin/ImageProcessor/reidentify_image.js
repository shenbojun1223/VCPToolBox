...
// JSON 真相源（multimodal-config.json）：优先于 config.env，支持热更新
let multiModalConfigStore = null;
try {
    multiModalConfigStore = require('../../modules/multiModalConfigStore.js');
} catch (storeError) {
    // 在管理面板手动重识别场景下若 store 不可用，回退到 ENV
    multiModalConfigStore = null;
}

function resolveMultiModalConfig() {
    const apiKey = process.env.API_Key;
    const apiUrl = process.env.API_URL;
    let multiModalModelName = process.env.MultiModalModel;
    let multiModalPromptText = process.env.MultiModalPrompt;
    let multiModalModelOutputMaxTokens = parseInt(process.env.MultiModalModelOutputMaxTokens, 10) || 50000;
    let multiModalModelThinkingBudget = parseInt(process.env.MultiModalModelThinkingBudget, 10);

    if (multiModalConfigStore) {
        try {
            const cfg = multiModalConfigStore.getConfig();
            if (cfg.MultiModalModel) multiModalModelName = cfg.MultiModalModel;
            if (cfg.MultiModalPrompt) multiModalPromptText = cfg.MultiModalPrompt;
            if (Number.isFinite(cfg.MultiModalModelOutputMaxTokens) && cfg.MultiModalModelOutputMaxTokens > 0) {
                multiModalModelOutputMaxTokens = cfg.MultiModalModelOutputMaxTokens;
            }
            if (Number.isFinite(cfg.MultiModalModelThinkingBudget)) {
                multiModalModelThinkingBudget = cfg.MultiModalModelThinkingBudget;
            }
        } catch (resolveErr) {
            console.error('[Reidentify] multiModalConfigStore.getConfig 失败，回退 ENV:', resolveErr.message);
        }
    }

    return { apiKey, apiUrl, multiModalModelName, multiModalPromptText, multiModalModelOutputMaxTokens, multiModalModelThinkingBudget };
}
...
async function reidentifyMediaByBase64Key(base64Key) {
   ...
   // 重新解析配置（JSON 真相源优先 + ENV 兜底），保证手动重识别命中最新热配置
   const {
       apiKey,
       apiUrl,
       multiModalModelName,
       multiModalPromptText,
   ...
   if (!apiKey || !apiUrl || !multiModalModelName || !multiModalPromptText) {
       throw new Error('错误：必要的 API 配置 (API_Key, API_URL, MultiModalModel, MultiModalPrompt) 未在 config.env 或 multimodal-config.json 中设置。');
   }
